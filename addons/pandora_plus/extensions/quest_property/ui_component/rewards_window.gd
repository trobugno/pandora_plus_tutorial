...
func _add_new_item():
	var scene = property_bar.get_scene_by_type("quest_reward_property")
	var control = scene.instantiate() as PandoraPropertyControl
	var item_property = PandoraProperty.new("", "array_item", "quest_reward_property")
	var new_reward = PPQuestReward.new()
	_items.append(new_reward)
	item_property.set_default_value(new_reward)
	_add_property_control(control, item_property, _items.size() - 1)
	item_added.emit(new_reward)
...
func open(original_rewards: Array):
	...
	_items = original_rewards.duplicate()
	_load_items()
